    ...
    <div class="card card-outline card-primary mb-4">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-striped small" id="dataTable{{ $department->id }}">
                    <thead>
                        <tr>
                            <th>ID Klaim</th>
                            <th>No ACC</th>
                    ...
                    <tbody>
                        @php
                        // Filter klaim untuk departemen ini
                        $deptClaims = $claims->whereIn('user_id', $department->users->pluck('id'));

                        // Kelompokkan klaim berdasarkan periode dan user
                        $claimsByPeriodAndUser = $deptClaims->groupBy(function($claim) {
                        return $claim->periode . '-' . $claim->user_id;
                        });

                        // Urutkan klaim berdasarkan tanggal pembuatan (terbaru dulu)
                        $sortedClaims = $deptClaims->sortByDesc('created_at');
                        ...
                        if ($saldoAwal < 0) $saldoAwal=0;
                            ...
                            if ($availableBalance < 0) $availableBalance=0;
                                ...
                                <td>
                                    <span class="d-inline-block text-truncate" style="max-width: 150px;" data-toggle="tooltip" title="{{ $claim->user->nama }}">
                                        {{ $claim->user->nama }}
                                    </span>
                                </td>
